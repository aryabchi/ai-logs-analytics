...
parse_task = Task(
    ...
    expected_output=(
        "Валидный JSON-массив объектов с полями: service, error_type, message, severity. "
...
report_task = Task(
    description=(
        ...
        "\nЗатем ВЫЗОВИ инструмент generate_markdown_report с текстом анализа."
    ),
    expected_output=(
        "Готовый отчёт в формате Markdown с разделами: Сводка инцидентов, Рекомендации. "
        "На русском языке, без вымышленных данных."
    ),
    agent=report_writer,
    context=[parse_task],  # ← результат парсинга → вход для генерации
    output_file="result/incident_report.md",  # ← автоматическое сохранение в каталог result/
)
